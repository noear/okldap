...
public class LdapSessionDefault implements LdapSession {
    ...
    public List<LdapPerson> findPersonList(String groupName) throws NamingException {
        ...
        if (group == null) {
            ...
            return findListBy("inetOrgPerson", "gidNumber=" + group.getGidNumber(), LdapPerson.class);
        }
    }
    ...
    public void createPerson(String groupName, LdapPerson person) throws NamingException {
        LdapGroup group = findGroupOne(groupName);
        if (group == null) {
            throw new IllegalGroupException("groupName: " + groupName);
        }
    ...
    public void updatePersonPassword(String userName, String userPasswordOld, String userPasswordNew) throws NamingException {
        LdapPerson user = findPersonOne(userName);
        if (user == null) {
            throw new IllegalPersonException("userName: " + userName);
        }

        if (user.verifyPassword(userPasswordOld) == false) {
    ...
    public LdapGroup findGroupOne(String groupName) throws NamingException {
        ...
        if (groupName.contains("=")) {
            groupCondition = groupName;
        } else {
            if (client.getGroupFilter() == null) {
                throw new IllegalConfigException("groupFilter");
            }

            if (client.getGroupFilter().contains("%s") == false) {
                throw new IllegalConfigException("groupFilter");
            }
    ...
    public <T extends LdapNode> List<T> findListBy(String objectClass, String condition, Class<T> tClass) throws NamingException {
        ...
        if (TextUtils.isNotEmpty(condition)) {
            filter.append("(")
                    .append(condition)
                    .append(")");
        }
